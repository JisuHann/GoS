...
import rvt.models.rvt_agent_gos as rvt_agent_gos
import rvt.models.eval_utils as eval_utils
import rvt.utils.ddp_utils as ddp_utils
import rvt.mvt.config_mask as mvt_cfg_mod

from rvt.mvt.mvt_gos import MVT
from rvt.models.rvt_agent import print_eval_log, print_loss_log
from rvt.utils.get_dataset import get_dataset_MASK
...
def train(agent, dataset, training_iterations,rank=0, epoch=0):
...
def eval(agent, dataset, iteration,rank=0, epoch=0):
    print(f"Start evaluating... Epoch {epoch}")
    log = defaultdict(list)
    
    agent.eval()
    data_iter = iter(dataset)
    iter_command = range(iteration)
    ...
    for iteration in tqdm.tqdm(
        ...
        log = agent.update(**update_args)
        for key, value in log.items():
            if 'vis' not in key:
                loss_log[key].extend(value)
            else:
                save_vis_img = log[key]
                   
        torch.cuda.synchronize()
        torch.cuda.empty_cache()
    for key, value in loss_log.items():
        loss_log[key]=sum(value)/len(value)
    loss_log['epoch'] = epoch
    if save_vis_img is not None:
        loss_log['eval_visualize'] = save_vis_img
    wandb.log(loss_log)
    print_eval_log(agent)
    del loss_log
    return 
...
def experiment(rank, cmd_args, devices, port):
    ...
    TRAINING_ITERATIONS = int(10000 // (exp_cfg.bs * len(devices)))
    EPOCHS = exp_cfg.epochs
    TRAIN_REPLAY_STORAGE_DIR = f"/home/guest/Desktop/RVT/replay_{NUM_TRAIN}/replay_train"
    # TRAIN_REPLAY_STORAGE_DIR = f"/home/guest/Desktop/RVT/replay_{NUM_TEST}/replay_val"
    TEST_REPLAY_STORAGE_DIR = f"/home/guest/Desktop/RVT/replay_{NUM_TEST}/replay_val"
    log_dir = get_logdir(cmd_args, exp_cfg)
    tasks = get_tasks(exp_cfg)
    print("Training on {} tasks: {}".format(len(tasks), tasks))

    t_start = time.time()
    get_dataset_func = lambda: get_dataset_MASK(
        tasks,
        BATCH_SIZE_TRAIN,
        BATCH_SIZE_TRAIN,
    ...
    train_dataset, eval_dataset = get_dataset_func()
    t_end = time.time()
    ...
    if exp_cfg.agent == "our":
        ...
        # if ddp:
        #     rvt = DDP(rvt, device_ids=[device],output_device=device,  static_graph=True)
        agent = rvt_agent_gos.RVTAgent(
            network=rvt,
            image_resolution=[IMAGE_SIZE, IMAGE_SIZE],
            add_lang=mvt_cfg.add_lang,
            scene_bounds=SCENE_BOUNDS,
            log_dir=f"{log_dir}/test_run/",
            cos_dec_max_step= TRAINING_ITERATIONS*40,
            **exp_cfg.peract,
            **exp_cfg.rvt,
        )
        print(sum(p.numel() for p in agent._network.parameters() if p.requires_grad))
    ...
    print("Loading ours?")
    breakpoint()
    agent_path = "/home/guest/Desktop/RVT/rvt/runs/gos/ours/model_41.pth"
    ...
    # wandb_mode = 'disabled'
    wandb_mode = 'online'
    name = f'6x6gos/{exp_cfg.bs}/{NUM_TRAIN}/feat{mvt_cfg.attn_dim}/ours'
    wandb.init(project='Multi-task(7)', config=exp_cfg, mode=wandb_mode, group=name)
    ...
    while True:
        ...
        out = train(agent, train_dataset, TRAINING_ITERATIONS, rank, epoch=i)
        eval(agent, eval_dataset, int(TRAINING_ITERATIONS//2), rank, epoch=i)
        if rank == 0:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--with-eval", action="store_true", default=True)
