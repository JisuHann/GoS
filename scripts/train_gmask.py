...
import rvt.models.rvt_agent_gmask as rvt_agent_gmask
...
from rvt.mvt.mvt_gmask import MVT
from rvt.models.eval_utils import print_eval_log, print_loss_log
from rvt.utils.get_dataset import get_dataset_GMASK
...
def experiment(rank, cmd_args, devices, port):
    ...
    t_start = time.time()
    get_dataset_func = lambda: get_dataset_GMASK(
        tasks,
        BATCH_SIZE_TRAIN,
        BATCH_SIZE_TRAIN,
    ...
    if exp_cfg.agent == "our":
        ...
        agent = rvt_agent_gmask.RVTAgent(
            network=rvt,
    ...
    # wandb_mode = 'disabled'
    wandb_mode = 'online'
    name = f'6x6gmask/{exp_cfg.bs}/{NUM_TRAIN}'
    ...
    while True:
        ...
        out_eval = eval(agent, eval_dataset, int(TRAINING_ITERATIONS), rank, epoch=i)
        if rank == 0:
