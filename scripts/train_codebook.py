...
import rvt.models.rvt_agent as rvt_agent_codebook
import rvt.mvt.config_patch as mvt_cfg_mod

from rvt.mvt.mvt_codebook import MVT
from rvt.models.rvt_agent import print_eval_log, print_loss_log
from rvt.utils.get_dataset import get_dataset_CODEBOOK
...
def experiment(rank, cmd_args, devices, port):
    ...
    t_start = time.time()
    get_dataset_func = lambda: get_dataset_CODEBOOK(
        tasks,
        BATCH_SIZE_TRAIN,
        BATCH_SIZE_TRAIN,
    ...
    if exp_cfg.agent == "our":
        ...
        agent = rvt_agent_codebook.RVTAgent(
            network=rvt,
    ...
    # wandb_mode = 'disabled'
    wandb_mode = 'online'
    name = f'7x7codebook/{exp_cfg.bs}/{NUM_TRAIN}/feat{mvt_cfg.attn_dim}'
    ...
    while True:
        ...
        eval(agent, eval_dataset, int(TRAINING_ITERATIONS//5), rank, epoch=i)
        if rank == 0:
