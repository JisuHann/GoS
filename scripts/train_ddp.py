...
import rvt.models.rvt_agent as rvt_agent_mask
...
from rvt.utils.get_dataset import get_dataset_PATCH
...
def experiment(rank, cmd_args, port):
    """experiment.
    ...
    TRAINING_ITERATIONS = int(43000 // (exp_cfg.bs * len(devices)))
    EPOCHS = exp_cfg.epochs
    TRAIN_REPLAY_STORAGE_DIR = "/home/guest/Desktop/RVT/replay_mask/replay_train"
    # TRAIN_REPLAY_STORAGE_DIR = "/home/guest/Desktop/RVT/replay/replay_train"
    TEST_REPLAY_STORAGE_DIR = "/home/guest/Desktop/RVT/replay_mask/replay_val"
    log_dir = get_logdir(cmd_args, exp_cfg)
    tasks = get_tasks(exp_cfg)
    print("Training on {} tasks: {}".format(len(tasks), tasks))

    get_dataset_func = lambda: get_dataset_PATCH(
        tasks,
        BATCH_SIZE_TRAIN,
        BATCH_SIZE_TRAIN,
    ...
    if exp_cfg.agent == "our":
        ...
        # rvt = rvt.to(torch.float32)
        print(f"DEVICE IS {rank}, {torch.cuda.current_device()}")
        rvt = nn.parallel.DistributedDataParallel(rvt, device_ids=[torch.cuda.current_device()],
                                            output_device=rank, 
                                            static_graph=True)
        agent = rvt_agent_mask.RVTAgent(
            ...
            cos_dec_max_step=TRAINING_ITERATIONS,
            **exp_cfg.peract,
            **exp_cfg.rvt,
        )
    ...
    # wandb_mode = 'online'
    name = 'patch'
    wandb.init(project='Multi-view multi-task', config=exp_cfg, mode=wandb_mode, group=name)
    ...
    while True:
        ...
        out_eval = eval(agent, eval_dataset, int(TRAINING_ITERATIONS/5), rank, epoch=i)
...
def setup_DDP():
    ...
    dist.init_process_group("nccl",timeout=datetime.timedelta(seconds=72000000), rank=rank, world_size=world_size)
