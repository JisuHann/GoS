...
class MVT(nn.Module):
    def __init__(
        ...
        spatial_size = img_size // self.img_patch_size  # 192 / 32 = 6
    ...
    def forward(
        ...
        **kwargs,
        ...
        rgb, mask = obs
        bs, num_img, num_mask, _, h, w = mask.shape
        ...
        for self_attn, self_ff in self.layers[self.depth[2]:]:
            x = self_attn(x) + x
            x = self_ff(x) + x
            
        # remove language features as sequence
        imgx = x[:, 1:,:]
        imgx = torch.mean(imgx.reshape(bs, num_img, num_mask, -1), dim=2)
        imgx = self.fc_aft_attn(imgx)
        imgx = imgx.reshape(bs, num_img, num_pat_img, num_pat_img, self.input_dim_before_seq) 
        imgx = rearrange(imgx, "b i p1 p2 d -> b i d p1 p2") 
