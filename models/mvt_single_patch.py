...
class MVT(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        if clip_emb==True:
            clip_attention = F.interpolate(clip_attention, size=(192,192), mode='bilinear', align_corners=True)
            clip_attention = clip_attention.reshape(clip_attention.shape[0], clip_attention.shape[1],img.shape[-2], img.shape[-1])
            clip_attention = rearrange(clip_attention, "b i h w -> (b i) 1 h w")
            img *= clip_attention
